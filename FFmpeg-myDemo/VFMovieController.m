...
#import "VFMovieDecoder.h"
#import "VFAudioManager.h"
#import "VFMovieGLView.h"

NSString * const VFMovieMinBufferedDuration = @"VFMovieMinBufferedDuration";
...
@interface VFMovieController () {

    VFMovieDecoder      *_decoder;
...
@property (readwrite, strong) VFArtworkFrame *artworkFrame;
@end

@implementation VFMovieController
...
{
    id<VFAudioManager> audioManager = [VFAudioManager audioManager];
    [audioManager activateAudioSession];
    return [[VFMovieController alloc] initWithContentPath: path parameters: parameters];
}
...
//        self.wantsFullScreenLayout = YES;
        ...
        __weak VFMovieController *weakSelf = self;
        
        VFMovieDecoder *decoder = [[VFMovieDecoder alloc] init];
        
        decoder.interruptCallback = ^BOOL(){
            
            __strong VFMovieController *strongSelf = weakSelf;
...
- (void)didReceiveMemoryWarning
{
    ...
    if (self.playing) {
        ...
        if (_maxBufferedDuration > 0) {
            ...
            [self play];
            /// didReceiveMemoryWarning, disable buffering and continue playing
...
- (void) setMovieDecoder: (VFMovieDecoder *) decoder
               withError: (NSError *) error
{
    ...
    if (!error && decoder) {
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: VFMovieMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: VFMovieMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: VFMovieDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
        ...
        /// "buffered limit: %.1f - %.1f", _minBufferedDuration, _maxBufferedDuration
...
- (void) setupPresentView
{
    ...
    if (_decoder.validVideo) {
        _glView = [[VFMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
    }
    
    if (!_glView) {
        /// fallback to use RGB video frame and UIKit
            ...
            if (!_currentAudioFrame) {
                ...
                @synchronized(_audioFrames) {
                    ...
                    if (count > 0) {
                        ...
                        VFAudioFrame *frame = _audioFrames[0];
...
- (BOOL)addFrames:(NSArray *)frames
{
    if (_decoder.validVideo) {
        ...
        @synchronized(_videoFrames) {
            ...
            for (VFMovieFrame *frame in frames)
        ...
        if (!_decoder.validVideo) {
            ...
            for (VFMovieFrame *frame in frames)
                if (frame.type == KxMovieFrameTypeArtwork)
                    self.artworkFrame = (VFArtworkFrame *)frame;
        }
    }
...
- (void)asyncDecodeFrames
{
    ...
    __weak VFMovieController *weakSelf = self;
    __weak VFMovieDecoder *weakDecoder = _decoder;
    ...
    dispatch_async(_dispatchQueue, ^{
        ...
        while (good) {
            ...
            @autoreleasepool {
                ...
                __strong VFMovieDecoder *decoder = weakDecoder;
...
- (CGFloat) tickCorrection
{
    ...
    if (correction > 1.f || correction < -1.f) {
        /// tick correction reset correction
        correction = 0;
        _tickCorrectionTime = 0;
    }
...
- (CGFloat)presentVideoFrame:(VFVideoFrame *)frame
{
    if (_glView) {
        ...
        VFVideoFrameRGB *rgbFrame = (VFVideoFrameRGB *)frame;
...
- (void)presentSubtitles
{
    ...
    if ([self subtitleForPosition:_moviePosition
        ...
        if (actual.count) {
            ...
            for (VFSubtitleFrame *subtitle in actual.reverseObjectEnumerator) {
                    ...
                    outdated: (NSArray **) pOutdated
{
    ...
    for (VFSubtitleFrame *subtitle in _subtitles) {
